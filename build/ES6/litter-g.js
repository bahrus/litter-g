import{observeCssSelector}from"./node_modules/xtal-latx/observeCssSelector.js";import{define}from"./node_modules/xtal-latx/define.js";export class LitterG extends observeCssSelector(HTMLElement){static get is(){return"litter-g"}insertListener(e){if(e.animationName===LitterG.is){const target=e.target;setTimeout(()=>{this.registerScript(target)},0)}}updateProps(props,target){props.forEach(prop=>{const initVal=target[prop];Object.defineProperty(target,prop,{get:function(){return this["_"+prop]},set:function(val){this["_"+prop]=val;if(this.renderer&&this.input)this.renderer(this.input,target)},enumerable:!0,configurable:!0});target[prop]=initVal;if(initVal!==void 0){}})}addProps(target){if(target.dataset.addedProps)return;this.updateProps(["input","renderer"],target);target.dataset.addedProps="true";if(!target.input){const inp=target.dataset.input;if(inp){target.input=JSON.parse(inp)}}}registerScript(target){this.addProps(target);if(!target.firstElementChild){setTimeout(()=>{this.registerScript(target)},50);return}const srcS=target.firstElementChild;if("script"!==srcS.localName)throw"Expecting script child";const script=document.createElement("script");let importPaths=`
